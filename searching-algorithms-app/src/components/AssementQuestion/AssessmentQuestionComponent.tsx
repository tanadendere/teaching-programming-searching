...
export default function AssessmentQuestionComponent({questionNumber, questionText, answerOne, answerTwo, answerThree, answerFour, correctAnswerNumber}: AssessmentQuestionDetails) {
    ...
    const determineButtonClass = (answerNumber: any) => {
        if (selectedAnswer === answerNumber && selectedAnswer !== correctAnswerNumber) {
    ...
    return (
        <div style={{ maxWidth: '600px', margin: '0 auto', padding: '20px' }}>
            ...
            <div className={styles.answerCard}>
                <button
                    className={`${styles.answerButton} ${determineButtonClass(1)}`}
                    onClick={() => handleAnswerClick(1)}
                >
                    {answerOne}
                </button>
                <button
                    className={`${styles.answerButton} ${determineButtonClass(2)}`}
            ...
            <div className={styles.answerCard}>
                <button
                    className={`${styles.answerButton} ${determineButtonClass(3)}`}
                    onClick={() => handleAnswerClick(3)}
                >
                    {answerThree}
                </button>
                <button
                    className={`${styles.answerButton} ${determineButtonClass(4)}`}
            ...
            <Dialog open={openFeedback} onClose={handleClose}>
                <DialogTitle>
                    ...
                    {selectedAnswer !== correctAnswerNumber &&
                        <div style={{ color: 'red' }}><strong>Wrong Answer. Try Again!</strong></div>}
                </DialogTitle>
